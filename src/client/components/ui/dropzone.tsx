...
>(({ className, ...props }, ref) => {
  ...
  const onRemoveFile = useCallback(
    async () => await cOnRemoveFile(fileId),
    [fileId, cOnRemoveFile],
  );
  const onRetry = useCallback(
    async () => await cOnRetry(fileId),
    [fileId, cOnRetry],
  );
...
interface DropzoneRemoveFileProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  children?: React.ReactNode;
}
...
interface DropzoneRetryFileProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  children?: React.ReactNode;
}
